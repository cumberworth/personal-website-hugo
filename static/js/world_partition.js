...
// Deep atlas blue ocean
const OCEAN_COLOR = [60, 115, 168];

// Generate N perceptually-spread map colours using golden-ratio hue steps.
function generateMapColors(n) {
  const colors = [];
  const goldenAngle = 137.508;
  // Start at green; golden-ratio steps spread hues evenly across the wheel
  const startHue = 115;
  for (let i = 0; i < n; i++) {
    const h = (startHue + i * goldenAngle) % 360;
    // Alternate lightness levels for contrast between adjacent regions
    const l = i % 2 === 0 ? 66 : 74;
    const s = 50;
